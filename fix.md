...
## Two traps in the shared tree, found the hard way on 2026-09-21

**1. The retelling gate was green against a stale edition.**
`ktretellcheck.py` checks Book One against
`work/rohonc/translation/rohonc_readers_edition.md`, which is generated by
`ktreader.py` from `proposals.json`. `ktcommit.sh` regenerates
`kttranslate.txt` before the checkers — that fix already exists, and the
comment in the script explains why — but it does **not** regenerate the
reader's edition. So through three hardening batches the edition still carried
`with_all_thy_heart` twenty times, a reading that had been withdrawn, and the
gate passed the whole time because it was comparing the prose against the same
stale artefact.

This is the failure METHOD.md already names once, in a different file: *a
generated figure is only as fresh as the artefact it is generated from; if a
checker reads a saved run, something must regenerate that run in the same
breath, or the check is a mirror.* It had a second home and nobody had
looked.

Regenerating it immediately produced four real failures in Part VII, all from
one hardening: the sign read as *son* on 064r–064v is now **shepherd**, which
is plainly right in the Good Shepherd parable and was plainly wrong before.
Three quotations and one editorial sentence were repaired. **Run
`python3 ktreader.py` before trusting a green retelling gate**, or better,
add it to `ktcommit.sh` beside the `kttranslate.py` line.

Note what the gate did and did not catch. It caught all three *quotations*
that said «good son». It did **not** catch the editorial sentence beside them
...
sitting uncommitted in the tree when a Book One commit ran, and it went in
under the wrong message. Nothing was lost and nothing was corrupted, but the
history now attributes that withdrawal to a commit about `whatbroke.py`. If
two people or two sessions are working at once, commit your own paths
explicitly, or expect the next `ktcommit.sh` to sweep the tree.
