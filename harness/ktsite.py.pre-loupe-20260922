...
# The project's own working documents. They are DESCRIBED here, not published:
...
.strip{margin:1.6em -64px;padding:18px 0;background:var(--paper2);overflow:hidden;position:relative}
.strip .rail{display:flex;width:max-content;gap:34px;animation:rohroll 90s linear infinite}
.strip:hover .rail{animation-play-state:paused}
.strip .it{display:flex;flex-direction:column;align-items:center;gap:5px;min-width:74px}
...
@keyframes rohroll{from{transform:translateX(0)}to{transform:translateX(-50%)}}
@media (prefers-reduced-motion:reduce){.strip .rail{animation:none}.strip{overflow-x:auto}}
.shot{margin:1.6em 0}
...
/* the reader */
#rdrwrap{border:1px solid var(--line);border-radius:3px;background:#fffdf8;margin:1.4em 0}
#rdrbar{display:flex;align-items:center;gap:12px;padding:8px 10px;border-bottom:1px solid var(--line)}
#rdrbar button{font:inherit;font-size:22px;line-height:1;width:40px;height:40px;border:1px solid var(--line);border-radius:3px;background:var(--paper2);color:var(--ink);cursor:pointer}
#rdrbar button:hover{border-color:var(--rub);color:var(--rub)}
#rloc{font-size:15px;color:var(--soft);margin:0 auto}
#rdr{height:72vh;min-height:420px}
/* the script page */
...
def strip_html(rows, sg, n=54):
    """The moving rail of signs. Rendered into the page, not fetched: it has to be
    there in the first frame, and it is the first thing anyone will look at."""
    ...
    it = "".join(
        ...
        f'<b>{html.escape(r["gloss"])}</b>'
        f'<span>{html.escape(" ".join(r["code"][i:i+3] for i in range(0, len(r["code"]), 3)))}</span></span>'
        for r in pick if sign_svg(r["code"], sg, 46))
    return f'<div class="strip"><div class="rail">{it}{it}</div><div class="fade"></div></div>'
...
CODE_GROUPS = [
    ("Reading the book", ["kttranslate.py", "ktreader.py", "ktcontext.py", "ktgap.py",
                          "ktnext.py", "ktlook.py", "ktaffix.py", "ktbook.py"]),
    ("The two rules that worked", ["ktname.py", "ktsegment.py", "ktvariant.py"]),
    ("The tests", ["ktheldout.py", "ktsentence.py", "ktorder.py", "ktblind.py",
                   "ktbootstrap.py", "ktpassid.py", "ktpassid2.py", "ktnullreplay.py",
                   "ktnullreplay2.py", "ktnullreplay3.py", "ktstrict.py", "ktcensus.py",
                   "ktblindfill.py", "kttestlib.py"]),
    ("Attempts that failed", ["ktextend.py", "ktalign.py", "ktdistrib.py", "ktread.py",
                              "ktreject.py", "ktmorph.py", "ktanchor.py", "ktverse.py",
                              "ktverse2.py", "ktsense.py", "ktgrammar.py", "ktscanmap.py"]),
    ("Checking ourselves", ["check_rohonc.py", "check_results.py", "gate.py",
                            "ktretellcheck.py", "ktnotecheck.py", "ktagree.py",
                            "keycheck.py", "whatbroke.py", "ktharden.py"]),
    ("The site and the outside readers", ["ktsite.py", "ktsitecopy.py", "ktsigns.py", "ktor.py"]),
]


def code_files(out):
    """Publish the programs themselves as plain text, and read each one's first
    docstring line so the listing says what it does. Served as .txt because the
    static handler has no rule for .py, and a download prompt is not a link."""
    src = os.path.join(ROOT, "harness")
    os.makedirs(os.path.join(out, "code"), exist_ok=True)
    seen, listed = {}, set()
    for fn in sorted(os.listdir(src)):
        if not fn.endswith(".py") or ".pre-" in fn:
            continue
        txt = read(os.path.join(src, fn))
        m = re.match(r'\s*"""(.*?)(?:\n|""")', txt, re.S)
        one = " ".join((m.group(1) if m else "").split())
        one = re.sub(r"^%s\s*--\s*" % re.escape(fn), "", one)
        open(os.path.join(out, "code", fn + ".txt"), "w", encoding="utf-8").write(txt)
        seen[fn] = one or "no description"
    groups = []
    for title, names in CODE_GROUPS:
        items = [(n, seen[n]) for n in names if n in seen]
        listed.update(n for n, _ in items)
        if items:
            groups.append((title, items))
    rest = [(n, d) for n, d in sorted(seen.items()) if n not in listed]
    if rest:
        groups.append(("Everything else", rest))
    return groups


def page_code(groups):
    out = []
    for title, items in groups:
        out.append(f"<h2>{html.escape(title)}</h2><ul>")
        for n, d in items:
            out.append(f'<li><a href="/rohonc/code/{html.escape(n)}.txt"><code>{html.escape(n)}</code></a> '
                       f'<span class="nof">{html.escape(d[:190])}</span></li>')
        out.append("</ul>")
...
<div id="rdrwrap">
  <div id="rdrbar">
    ...
    <span id="rloc">opening the book…</span>
    <button id="rnext" aria-label="Next page">&#8250;</button>
  </div>
  <div id="rdr"></div>
</div>
<!-- the same two files the house reader at /read/reader.php loads; cdnjs has no 0.3.x epub.js -->
<script src="https://cdn.jsdelivr.net/npm/jszip@3.10.1/dist/jszip.min.js"></script>
<script src="https://cdn.jsdelivr.net/npm/epubjs@0.3.93/dist/epub.min.js"></script>
<script>
(function(){
  var el=document.getElementById('rdr'), loc=document.getElementById('rloc');
  if(!window.ePub){ loc.textContent='The reader could not load. Use the EPUB link above.'; return; }
  var book=ePub('/rohonc/book/the-rohonc-codex.epub'), rend=book.renderTo(el,{width:'100%',height:'100%',flow:'paginated',spread:'auto'});
  rend.themes.default({'body':{'background':'#f5efe3','color':'#1d1a16'},
                       'p':{'font-family':'"EB Garamond",Garamond,serif','line-height':'1.55'}});
  rend.display().then(function(){ loc.textContent=''; });
  book.loaded.navigation.then(function(){ return book.locations.generate(1600); })
      .then(function(){ show(rend.currentLocation()); });
  function show(l){ if(!l||!l.start) return;
    var p=book.locations.percentageFromCfi(l.start.cfi);
    loc.textContent = (l.start.displayed && l.start.displayed.page ? '' : '') +
      (isNaN(p)?'':Math.round(p*100)+'% through');
  }
  rend.on('relocated', show);
  document.getElementById('rprev').addEventListener('click',function(){rend.prev();});
  document.getElementById('rnext').addEventListener('click',function(){rend.next();});
  document.addEventListener('keydown',function(e){
    if(e.key==='ArrowLeft'){rend.prev();} if(e.key==='ArrowRight'){rend.next();}});
})();
</script>
"""
    return shell("read", "Read it here",
...
def page_data(files, runs, outside):
    ...
    body = ["<h1>Data</h1>",
            ...
            '<p>What is not here, and why, is under <a href="/rohonc/sources.html">Sources</a>.</p>',
            "<h2>The readings and the rendering</h2><ul>"]
    for href, name, desc, p in files:
        body.append(li(href, name, desc, sz(p)))
    body.append("</ul><h2>The working documents</h2>")
    body.extend(paras("docs_lead"))
    body.append("<ul class=\"steps\">" + docs_html() + "</ul>")
    body.append(f'<p>Ask for any of them at <a href="mailto:{CONTACT}">{CONTACT}</a>. '
                f'What they say is on the <a href="/rohonc/method.html">method</a>, '
                f'<a href="/rohonc/tests.html">tests</a> and '
                f'<a href="/rohonc/sources.html">sources</a> pages.</p>')
...
def main(argv):
    ...
    sp = os.path.join(WORK, "signs.json")
    if os.path.isfile(sp):
        copy(sp, os.path.join(out, "data", "signs.json"))
    ...
    w("code.html", page_code(code_files(out)))
