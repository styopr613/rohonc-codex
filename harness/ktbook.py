...
Shape: two books between one pair of covers, and the split is the whole point.
  ...
  BOOK ONE is the reading experience. One continuous retelling per part,
  written as prose, labelled editorial on every part, citing the folios each
  sentence draws on. It is allowed to smooth. It is not allowed to know
  anything the gloss does not contain, and ktretellcheck.py refuses it if it
  does.
...
def retelling():
    ...
    for m in re.finditer(r"^## ([^\n]+)\n\n### folios (\d{3}[rv])–(\d{3}[rv])\n"
                         r"(.*?)(?=^## |\Z)", open(RT, encoding="utf-8").read(),
                         re.M | re.S):
...
script, in the library of the Hungarian Academy of Sciences. For roughly two
hundred years nobody could read it. It dates itself: 1593.
...
apocryphal Life of Adam and Eve running straight into the legend of the Rood,
the story of Joachim and Anne, a Passion, sermons and parables for the gospels
of the year, the Finding of the Cross, the Acts of the Apostles, and at the
very end a few pages of one man's diary.

**Everything you can read here rests on their work.** Roughly three quarters
of the words below are theirs or follow directly from theirs. Kiraly and Tokai
have read large parts of this manuscript. What has not existed before is the
whole of it, end to end, in a modern language, with every word marked for how
well it is known -- and that, and only that, is what this edition claims to
be. Their own translation is unpublished; this is not it, and nothing here
should be attributed to them except the dictionary and the grammar, which are
entirely theirs.
...
gloss because it is one. The paragraph at the head of each folio was written
from that folio's gloss and from nothing else -- not from the passage the
folio cites, not from the chapter heading, not from the page image -- and was
then checked word by word against that same gloss. Eight folios failed that
check twice; their English was thrown away and they print their marked lines
alone.

Book One is the opposite kind of document, and says so on its own first page.
...
It does not bear on the 93.3% that are read, which rest on the dictionary and
...
a worse witness to what a Catholic in Hungary in 1593 was actually reading."""},
        {"type": "appendix", "pos": 3, "title": "Acknowledgment",
...
def build():
    ...
    for bi, (start, pname, pblurb) in enumerate(bounds):
        ...
        if first:
            ...
            ch["part_sub"] = (
                "A continuous retelling, part by part. It is editorial: it "
                "joins, it orders and it supplies the English that the "
                "manuscript's word order does not. Every sentence is drawn "
                "from the folios cited beside it, and words inside italic "
                "quotation marks are the manuscript's own as Book Two renders "
                "them. Nothing here comes from the sources the codex is "
                "working from. Where this and Book Two disagree, Book Two is "
                "right.")
            first = False
        chapters.append(ch)
    ...
    for bi, (start, pname, pblurb) in enumerate(bounds):
        ...
        for ci in range(0, len(run), PER_CHAPTER):
            ...
            for pg, title, prose, lines in grp:
                ...
                body.append("")
                # The English written from the gloss alone and leak-checked.
                # Eight folios lost theirs and print their lines bare; that is
                # what a refused folio is supposed to look like.
                if pg in eng:
                    body.append(eng[pg].replace("\n", " "))
                    body.append("")
                if lines:
                    body.append("<pre>")
            ...
            if ci == 0:
                ...
                ch["part_sub"] = (
                    ...
                    "to a chapter. The paragraph under each folio number is "
                    "English written from that folio's gloss and nothing else; "
                    "the lines beneath it are the manuscript itself, line for "
                    "line, with every mark intact. This is the evidence."
                    if bi == 0 else pblurb)
            chapters.append(ch)
    return {
