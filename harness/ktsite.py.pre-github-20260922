...
def crosswalk(out):
    ...
    def plink(n):
        return (f'<a href="/rohonc/code/{html.escape(n)}.txt"><code>{html.escape(n)}</code></a>'
                if os.path.isfile(os.path.join(out, "code", n + ".txt")) else f"<code>{html.escape(n)}</code>")
    def rlink(n):
        return (f'<a href="/rohonc/data/runs/{html.escape(n)}">{html.escape(n)}</a>'
                if os.path.isfile(os.path.join(out, "data", "runs", n)) else html.escape(n))
    body = ""
...
def code_files(out):
    """Publish the programs the results page cites, as plain text. Served as
    .txt because the static handler has no rule for .py. A file served by an
    earlier build and no longer cited is removed, by name."""
    src = os.path.join(ROOT, "harness")
    dst = os.path.join(out, "code")
    os.makedirs(dst, exist_ok=True)
    want, _ = cited()
    for fn in sorted(os.listdir(dst)):
        if fn.endswith(".txt") and fn[:-4] not in want:
            os.remove(os.path.join(dst, fn))
    out_list = []
    for fn in sorted(want):
        if os.path.isfile(os.path.join(src, fn)):
            open(os.path.join(dst, fn + ".txt"), "w", encoding="utf-8").write(read(os.path.join(src, fn)))
            out_list.append(fn)
    return out_list
...
def page_code(out):
    ...
    body = ["<h1>The programs</h1>", paras("code_lead", "lead"),
            ...
            crosswalk(out),
            "<p>These are the programs the results rest on. The whole harness, with its history, goes up "
            "as a public repository on GitHub; until it does, nothing else is listed here, because a "
            "hand-kept list of files is stale the first week the code is touched.</p>"]
...
def page_data(files, runs, outside):
    ...
    body.append('<p>The standing orders are published whole at <a href="/rohonc/orders.html">The standing orders</a>. '
                'The rest go up with the public repository on GitHub, with their history. What they say is on the '
                '<a href="/rohonc/method.html">method</a>, <a href="/rohonc/tests.html">tests</a> and '
                '<a href="/rohonc/sources.html">sources</a> pages.</p>')
...
def main(argv):
    ...
    runs = []
    _, want_runs = cited()
    rdir = os.path.join(out, "data", "runs")
    for fn in sorted(os.listdir(rdir)):
        if fn not in want_runs:
            os.remove(os.path.join(rdir, fn))
    for fn in sorted(want_runs):
        if os.path.isfile(os.path.join(WORK, fn)):
            copy(os.path.join(WORK, fn), os.path.join(rdir, fn))
            runs.append((f"/rohonc/data/runs/{fn}", fn, os.path.join(rdir, fn)))
