...
BOOKS = ("Genesis Exodus Leviticus Numbers Deuteronomy Joshua Judges Ruth Samuel Kings "
         "Chronicles Ezra Nehemiah Esther Job Psalms Psalm Proverbs Ecclesiastes Isaiah "
         "Jeremiah Lamentations Ezekiel Daniel Hosea Joel Amos Jonah Micah Zechariah Malachi "
         "Matthew Mark Luke John Acts Romans Corinthians Galatians Ephesians Philippians "
         "Colossians Thessalonians Timothy Hebrews James Peter Jude Revelation").split()
ALIAS = {"Psalm": "Psalms"}

REF = re.compile(r"\b(%s)\b\.?\s*(\d+)(?::(\d+)(?:\s*[-–]\s*(\d+))?)?|(?<![\d:])(\d+):(\d+)(?:\s*[-–]\s*(\d+))?"
                 % "|".join(BOOKS))
...
def verses():
    ...
    if not os.path.exists(KJV):
        return out
    book, heading, cur, buf = None, "", None, []
    for raw in open(KJV, encoding="utf-8", errors="ignore"):
        line = raw.rstrip("\n")
...
def short(heading):
    h = heading.replace("The Revelation of Saint John the Devine", "Revelation")
    h = re.sub(r"^The .*?Called ", "", h)
    h = re.sub(r"^The Gospel According to Saint ", "", h)
    h = re.sub(r"^The Acts of the Apostles", "Acts", h)
    h = re.sub(r"^The (General )?Epistle .* to the ", "", h)
    h = re.sub(r"^The (General )?Epistle .* of ", "", h)
    h = re.sub(r"^The (First|Second|Third) Book of (the )?", "", h)
    h = re.sub(r"^The Book of (the )?", "", h)
    h = re.sub(r"^The (Song of |Lamentations of |Proverbs)", lambda m: m.group(0)[4:], h)
    h = re.sub(r"^The ", "", h)
    return h.split()[-1] if h.split() else h
...
def refs_in(note):
    ...
    for m in REF.finditer(note):
        if m.group(1):
            ...
            a = int(m.group(6))
            b = int(m.group(7)) if m.group(7) else a
            out.append((book, int(m.group(5)), a, b))
    return out
...
def cross(gl, doc, seg, var, prop, inv, idx, vv, cited, cnt, wide):
    ...
    for pg, r in cited.items():
        pools[pg] = pool(" ".join(t for _, t in passage(vv, r, wide)), idx)
    rows = []
...
def onehole(gl, doc, seg, var, prop, inv, idx, vv, cited, cnt, wide):
    """Lines with one hole on a cited folio, smallest pool first."""
    rows = []
    for p in doc:
        if p.page not in cited:
            continue
        pl = pool(" ".join(t for _, t in passage(vv, cited[p.page], wide)), idx)
        for i, ln in enumerate(p.lines, 1):
            tk = [t for run in ln for t in run]
            miss = [A.strip(t)[0] for t in tk if A.strip(t)[0] not in inv]
            if len(miss) != 1:
            ...
            rows.append((len(forms), p.page, i, miss[0], forms, render(tk, miss[0], gl, seg, var, prop, inv)))
    rows.sort(key=lambda r: (r[0], r[1], r[2]))
    print(f"{len(rows)} one-hole lines on cited folios")
    for n, pg, i, b, forms, s in rows:
...
def main(argv):
    ...
    if want:
        for pg in want:
            ...
            fr = pool(" ".join(t for _, t in vs), idx)
            if vs:
                words = sorted(fr.items(), key=lambda x: -sum(x[1].values()))
                print(f"    {len(vs)} verse(s); {len(words)} content words with no sign yet:")
                print("    " + ", ".join("/".join(sorted(c)) for _, c in words))
    ...
    if "--cross" in argv:
        return cross(gl, doc, seg, var, prop, inv, idx, vv, cited, cnt, wide)
    if "--holes" in argv:
        return onehole(gl, doc, seg, var, prop, inv, idx, vv, cited, cnt, wide)
    ...
    for pg, r in cited.items():
        vs = passage(vv, r, wide)
        nverse += len(vs)
        for s, c in pool(" ".join(t for _, t in vs), idx).items():
            byword[s].add(pg)
            forms[s].update(c)
