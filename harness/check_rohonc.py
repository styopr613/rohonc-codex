...
def main():
    ...
    check("translation: the file exists and covers 194 folios",
          md.count("\n## 0") + md.count("\n## 1") == 194, str(md.count("\n## 0")))
    import re as _re
    ...
    check("proposals: 2404 tokens = 8.0% rendered",
          len(g) == 2 and g[0] == 2404 and close(g[1], 8.0, .02), str(g))
    g = nums(kt, "lines with every word read", 2)
    check("proposals: 2574 lines fully read with them",
          len(g) == 2 and g[0] == 2574 and close(g[1], 58.9, .02), str(g))
    ...
    check("ROHONC: METHOD.md and the readings are stated",
          "METHOD.md" in flat and "195 signs are read" in flat
          and "23.8% to **58.9%**" in flat)
...
folios are translated" in doc and "names Cleopas" in flat
    ...
    check("ROHONC: the self-citations are checked, ten of eleven",
          "Ten of eleven land" in flat and "Luke 10:23   OK" in doc)
