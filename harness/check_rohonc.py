...
def main():
    ...
    check("proposals: 3622 tokens = 12.1% rendered",
          len(g) == 2 and g[0] == 3622 and close(g[1], 12.1, .02), str(g))
    g = nums(kt, "lines with every word read", 2)
    check("proposals: 3532 lines fully read with them",
          len(g) == 2 and g[0] == 3532 and close(g[1], 80.8, .02), str(g))
    ...
    check("ROHONC: METHOD.md and the readings are stated",
          "METHOD.md" in flat and "943 signs are read" in flat
