...
def main():
    ...
    check("proposals: 3615 tokens = 12.1% rendered",
          len(g) == 2 and g[0] == 3615 and close(g[1], 12.1, .02), str(g))
    g = nums(kt, "lines with every word read", 2)
    check("proposals: 3556 lines fully read with them",
          len(g) == 2 and g[0] == 3556 and close(g[1], 81.3, .02), str(g))
    ...
    check("ROHONC: METHOD.md and the readings are stated",
          "METHOD.md" in flat and "1778 signs are read" in flat
