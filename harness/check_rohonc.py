...
def main():
    ...
    check("translation: the file exists and covers 248 folios",
          md.count("\n## 0") + md.count("\n## 1") == 248, str(md.count("\n## 0")))
    import re as _re
    ...
    check("proposals: 2548 tokens = 8.5% rendered",
          len(g) == 2 and g[0] == 2548 and close(g[1], 8.5, .02), str(g))
    g = nums(kt, "lines with every word read", 2)
    check("proposals: 2663 lines fully read with them",
          len(g) == 2 and g[0] == 2663 and close(g[1], 60.9, .02), str(g))
    ...
    check("ROHONC: METHOD.md and the readings are stated",
          "METHOD.md" in flat and "267 signs are read" in flat
          and "23.8% to **60.9%**" in flat)
