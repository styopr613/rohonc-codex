...
def main():
    ...
    # `A and B or C` binds as `(A and B) or C`, so any stray "OFF" anywhere in
    # the document satisfied this on its own and the two claims it names were
    # never required. Both are required now.
    check("ROHONC: the self-citations are checked, seven of eight",
          "Matthew 5:13  OK" in doc and "John 3:1     OFF" in flat.replace("  ", " ").replace("John 3:1 OFF", "John 3:1     OFF"))
