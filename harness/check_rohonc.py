...
def main():
    ...
    check("proposals: 3657 tokens = 12.2% rendered",
          len(g) == 2 and g[0] == 3657 and close(g[1], 12.2, .02), str(g))
    g = nums(kt, "lines with every word read", 2)
    check("proposals: 3566 lines fully read with them",
          len(g) == 2 and g[0] == 3566 and close(g[1], 81.6, .02), str(g))
    ...
    check("ROHONC: METHOD.md and the readings are stated",
          "METHOD.md" in flat and "978 signs are read" in flat
