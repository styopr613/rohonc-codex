...
def main():
    ...
    check("proposals: 3631 tokens = 12.1% rendered",
          len(g) == 2 and g[0] == 3631 and close(g[1], 12.1, .02), str(g))
    g = nums(kt, "lines with every word read", 2)
    check("proposals: 3539 lines fully read with them",
          len(g) == 2 and g[0] == 3539 and close(g[1], 80.9, .02), str(g))
    ...
    check("ROHONC: METHOD.md and the readings are stated",
          "METHOD.md" in flat and "952 signs are read" in flat
          and "23.8% to **80.9%**" in flat)
