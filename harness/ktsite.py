...
(function(){
  ...
  function layout(){
    ...
    // the strip opens a few signs in, so the glass has signs on both sides of
    // it; from a standing start at the first sign the left half of the band is
    // empty and the strip does not look like something that moves
    if(!opened&&mid.length){opened=true;
      track.scrollLeft=mid[Math.min(+strip.dataset.open||0,mid.length-1)]-track.clientWidth/2;}
    cur=-1; draw();
  }
  function nearest(p){
  ...
  function draw(){
    ...
    if(i!==cur){
      ...
      // an arrow that cannot move the strip says so rather than doing nothing
      prev.disabled=(i===0); next.disabled=(i===mid.length-1);
    }
  }
  function tick(){ if(!raf) raf=requestAnimationFrame(draw); }
  function step(d){
    var i=nearest(track.scrollLeft+track.clientWidth/2)+d;
    if(i<0||i>=mid.length) return;
    track.scrollTo({left:mid[i]-track.clientWidth/2,behavior:calm.matches?'auto':'smooth'});
  }
  track.addEventListener('scroll',tick,{passive:true});
  ...
  // mouse drag only: a touch already scrolls the box natively, and handling it
  // here as well moved the strip at twice the speed of the finger
  ...
  track.addEventListener('pointermove',function(e){
    if(!down) return;
    track.scrollLeft=s0-(e.clientX-x0); e.preventDefault();
  });
  function release(){ if(down){down=false; track.classList.remove('grabbing');} }
