...
<p>The file itself, every entry with its full evidence, will be posted to GitHub with the rest of the data.</p>
<script>
(function(){{
var ROWS=[];var T='',Q='';
...
def page_code(out):
    ...
    body = ["<h1>The programs</h1>", paras("code_lead", "lead"),
            ...
            crosswalk(out),
            "<p>The programs and every saved run will be posted to GitHub, with their history. Nothing is served "
...
def page_data(outside):
    ...
    body = ["<h1>Data</h1>", paras("data_lead", "lead"),
            "<h2>The corpus</h2>",
            "<p>The books the codex is compiled from, which every folio is read against. All are public domain; "
            "the full list, with terms and the courtesy failures, is in the provenance file, and this list is read out of it.</p><ul class=\"steps\">"]
    for name, src in corpus_list():
        body.append(f"<li><b>{html.escape(name)}</b>" + (f' <span class="nof">{html.escape(src)}</span>' if src else "") + "</li>")
    ...
    body.append("<h2>Everything else</h2><p>The dictionary of added readings with its evidence, the folio English, the rendering, "
                "every saved run, the outside readers' replies, the working documents and the programs will be posted to GitHub, "
...
def main(argv):
    ...
    # dictionary page searches dictionary.json. Copies served by earlier builds are
    # taken down by name, so no link anywhere can point at a stale file.
    for sub in ("data/runs", "data", "code"):
